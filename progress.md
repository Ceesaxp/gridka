...
## 2026-02-26, 21:15 - US-106 - Add regression tests for async invalidation, teardown safety, and fetch-load coherence
- Created Tests/AsyncInvalidationRegressionTests.swift with 20 comprehensive regression tests covering all US-106 acceptance criteria:
  - **Stale fetch discard (AC-1):** `testStaleFetchDiscardedAfterRapidFilterChanges`, `testStaleFetchDiscardedAfterRapidSortToggling` — rapid filter/sort changes interleaved with fetch bursts, verifying all completions fire and stale results don't corrupt state
  - **RowCache non-mutation (AC-2):** `testRowCacheNotMutatedByObsoleteFetchResults` — dispatches burst fetches then immediately invalidates via sort change, verifies stale pages are NOT cached
  - **Deterministic totalFilteredRows (AC-3):** `testDeterministicFilteredRowCountAfterFilterAdd`, `testDeterministicFilteredRowCountAfterFilterRemove`, `testDeterministicCountAfterRapidFilterChanges`, `testSortChangeDoesNotRequeryCount`, `testDeterministicCountAfterSearchTermChange` — verifies row counts update via completion callbacks, not timing-based delays
  - **SparklineHeaderCell teardown (AC-4/5):** `testSparklineHeaderCellClearSummarySafety`, `testSparklineHeaderCellNilSummaryPreventsStaleDraws`, `testSparklineHeaderCellRapidSummaryReplacement`, `testRepeatedSessionCreateDestroySummaryLifecycle`, `testSummaryInvalidationDuringComputation` — verifies clearSummary nils snapshot, rapid replacement is consistent, and invalidation during in-flight computation discards stale results
  - **Scroll stress (AC-6):** `testScrollStressRapidPageFetchesNoCrash`, `testScrollWithConcurrentViewStateChanges` — 100+ page fetches with pseudo-random patterns and interleaved sort/filter/search mutations
  - **FetchingPages bookkeeping (AC-7):** `testFetchCompletionAlwaysFires`, `testFetchCompletionFiresForOutOfBoundsPages`, `testRowCachePageReplacementIsClean`, `testRowCacheInvalidateAllClearsEverything` — verifies completions always fire (even for stale results), enabling callers to clear fetchingPages bookkeeping
  - **Integration:** `testFullFilterLifecycleDeterministic` — full load, filter, verify count, clear filter, verify restored count, with cache state assertions at each step
- Fixed existing FileSessionCrashRegressionTests.swift compile error: `AggregationEntry(function:, columnName:)` to `AggregationEntry(columnName:, function:)` argument order
- Added both test files to Gridka.xcodeproj/project.pbxproj (PBXBuildFile, PBXFileReference, Tests group, test Sources build phase)
- Files changed: Tests/AsyncInvalidationRegressionTests.swift (new), Tests/FileSessionCrashRegressionTests.swift (fix), Gridka.xcodeproj/project.pbxproj, plans/prd.json
- Build succeeds, all 94 tests pass (20 new + 3 newly-registered existing + 71 previous)
- **Learnings for future iterations:**
  - Test files must be manually registered in pbxproj when xcodegen is not available — requires 4 edits: PBXBuildFile, PBXFileReference, group children, and Sources build phase
  - `FileSessionCrashRegressionTests.swift` was in Tests/ but never added to the project — it was compiled for the first time in this session, revealing the AggregationEntry argument order bug
  - XCTest expectations with `expectedFulfillmentCount` are the right pattern for verifying all async completions fire — each fetch completion calls `done.fulfill()`, and `wait(for:timeout:)` blocks until all are received
  - SparklineHeaderCell unit tests can run without an NSWindow/NSView hierarchy — the cell is a plain NSTableHeaderCell subclass and its properties can be tested directly
  - The `testSummaryInvalidationDuringComputation` test uses `DispatchQueue.main.asyncAfter(deadline: .now() + 10)` as a settling timeout since the invalidate+recompute cycle has no direct completion handler — this is acceptable for a regression test but a production callback would be cleaner
  - The `testRowCacheNotMutatedByObsoleteFetchResults` test validates the core US-101 generation token contract: burst pages dispatched before a sort change are NOT present in the cache after the change settles
----

## 2026-02-27, 13:05 - US-001 - Enforce computed expression safety on runtime add/apply path
- Enhanced `FileSession.containsSemicolonOutsideQuotes()` to handle SQL comments in addition to string literals:
  - `--` line comments: semicolons after `--` until newline are now accepted
  - `/* */` block comments: semicolons inside block comments are now accepted
  - Single-quoted string literals: unchanged (already handled)
- Added semicolon injection validation to `ComputedColumnPanelController.validateInput()`:
  - Calls `FileSession.containsSemicolonOutsideQuotes(expression)` -- same shared validator used by the preview path
  - Shows non-fatal error via existing `showError()` mechanism
  - Validation runs before panel close, preventing invalid expressions from reaching the runtime add/apply handler
- Files changed: Sources/Model/FileSession.swift, Sources/UI/ComputedColumnPanelController.swift, plans/prd.json
- Build succeeds, all 94 tests pass
- **Learnings for future iterations:**
  - `FileSession.containsSemicolonOutsideQuotes()` is a `static` method, callable from any module without an instance -- good pattern for shared validators
  - The preview path already had both semicolon validation AND a read-only transaction wrapper; the runtime path only needed the semicolon validation since DuckDB syntax errors are caught by the fetchPage call and rolled back
